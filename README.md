...
**Weekly digest schedule:** Vercel hits **`/api/cron/weekly-digest`** at **`0 12,13,14 * * *`** (12:00, 13:00, and 14:00 UTC — one cron definition, three invocations per day). In [`lib/digest/digestCronSchedule.ts`](lib/digest/digestCronSchedule.ts), `WEEKLY_DIGEST_CRON_SCHEDULES` lists the same three slots separately for “next digest” math; keep it in sync if you change `vercel.json`. Each user sets **weekday + hour** (America/Toronto) on the dashboard; a send only happens when the cron instant matches that **weekday and hour** locally (so 8 AM Eastern uses 12 UTC in EDT and 13 UTC in EST). **Minute** is for display only. Deduping: **at most once per calendar day** in the user’s timezone (`gmail_connections.weekly_digest_last_calendar_date`).
